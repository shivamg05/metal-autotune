...
timing. spikes/ramp_after_idle.py showed a 0.75s rest is not recoverable, so
...
against each other:
  ...
  soak        runs one arm only, to watch how far that regime throttles.
    ...
    uv run python spikes/rest_vs_norest.py --work-dir /tmp/rvn --cycles 10
    uv run python spikes/rest_vs_norest.py --work-dir /tmp/rvn2 --mode soak --arm norest
"""
...
def main() -> int:
    ...
    for cycle in range(args.cycles):
        ...
        for name in names:
            ...
            row = {
                "cycle": cycle, "arm": name, "bandwidth_gbps": round(bw, 1),
