...
def main(argv=None):
    ...
    parser.add_argument("--steps", type=int, default=final.get("steps", 20),
                        help="generated tokens for library inference, otherwise consecutive forward steps")
    ...
    settings = {"generated_tokens": args.steps} if metadata.get("use_library_inference") else {}
    ...
    for label, inputs, role in validator.saved_workloads(metadata):
        if role != "declared":
            continue
        runs = [make_sequence(step, inputs, args.steps) for step in (original, patched)]
        ...
        if metadata.get("use_library_inference"):
            ...
            runs = [lambda s=step: s.model.sequence(inputs, args.steps)
            ...
            if golden_file:
                if args.steps != final.get("steps", 20):
                ...
                observations = [validator.paced(lambda s=step: s.model.sequence(inputs, args.steps, include_state=True))
                                for step in (original, patched)]
            ...
            observations = [validator.paced(lambda s=step: sequence_observation(s, inputs, args.steps))
                            for step in (original, patched)]
            check = validator.check_outputs(*observations, label + ":sequence",
                                            **validator.policy_arguments(metadata))
            print(validator.describe(check))
            if not check["passed"]:
                return 1
        warm = [lambda s=step: s(*inputs) for step in (original, patched)]
        ...
        amount = f"{args.steps} generated tokens" if kind == "library_generation" else f"{args.steps} consecutive steps"
    ...
    if args.json:
        args.json.write_text(json.dumps({"steps": args.steps, "pairs": args.pairs, "workloads": rows,
