...
The briefing includes at most 12 selected attempts and six relevant lessons.
`experience_archive.source_id` gives read-only access to full history, every
candidate in this region, and all lessons from this job. Use `read_source` to
search for an earlier kernel/hypothesis id or lesson id, then follow the source
references in that record. Failures remain available alongside successes.
Lessons include the evaluated candidate and its parent as evidence; explanations
are the agent's interpretations, not established rules. Worked failure examples remain. Return actual Metal
...
opener used, unless it repairs a kernel that failed. Generate four approaches
from the code, shapes and measurements, explaining each mechanism, expected
benefit and concrete difference. Plan them and implement the first in the same
normal reply; revise untried plans as evidence arrives. `techniques_reference`
is optional reading, not a required menu. After the
four openers (budget permitting), each reply tests one coherent hypothesis.
It can refine any earlier candidate, repair a failure, combine ideas, or start
another design from the scaffold. Choosing a head does not close other designs.
`inspiration` rotates one earlier opener not already visible, with its outcome
and a small code excerpt. It may be slower or failed. Use it if useful; it does
not choose your parent. Its full kernel is available through source lookup. Explain
...
monitoring and reporting rules in [operator reporting guide](operating.md#7-how-to-talk-to-the-person).
